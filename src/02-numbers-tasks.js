...
function getAverage(v1, v2) {
  const sum = (v1 + v2) / 2;
  return sum === Number.POSITIVE_INFINITY ? Number.MAX_VALUE : sum;
}
/**
...
function roundToPowerOfTen(num, pow) {
  ...
  // const degree = num / 10 ** (pow - 1);
  // const value = Math.round(num / degree);
  // const finValue = value * degree;
  // return finValue;
  const degree = 10 ** pow;
...
function toNumber(value, def) {
  ...
  return Number.isNaN(check) ? def : check; // value
}
